...
complex*16, dimension(2,1):: fv,fv1
real*8, dimension(2048):: x,v,v1ad,v2ad,v11,v12,v22,f,d
...
k = 2*pi*(h1-1-2048)/(2048.*dx)
endif
return
end function k

subroutine vop(s,i1,f)
integer i1
complex*16, dimension(2,1):: s,s1,s2,f
complex*16, dimension(2,2):: va,vb
va(1,1) = exp(-iota*v11(i1)*dt/4.)
va(2,2) = exp(-iota*v22(i1)*dt/4.)
va(1,2) = cmplx(0.0,0.0)
va(2,1) = cmplx(0.0,0.0)
!print*, abs(s)**2
s1 = matmul(va,s)
!print*, abs(s1)**2
vb(1,1) = cos(v12(i1)*dt/2.)
vb(1,2) = -iota*sin(v12(i1)*dt/2.)
vb(2,1) = -iota*sin(v12(i1)*dt/2.)
vb(2,2) = cos(v12(i1)*dt/2.)
s2 = matmul(vb,s1)
f = matmul(va,s2)
end subroutine vop
